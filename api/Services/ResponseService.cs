...
namespace api.Services
{
    public class ResponseService : IResponseService
    {
        ...
        public async Task SaveSurveyResultAsync(SubmitSurveyResultDto dto)
        {
            ...
            // L?y d? li?u câu h?i và option ?? l?u n?i dung t?nh
            var questionDict = await _context.Questions.ToDictionaryAsync(q => q.Id);
            var optionDict = await _context.Options.ToDictionaryAsync(o => o.Id);

            var answers = dto.Answers.Select(ans =>
            {
                string? questionContent = questionDict.TryGetValue(ans.QuestionId, out var q) ? q.Content : null;
                string? answerContent = null;
                // SingleChoice
                if (ans.OptionId.HasValue && optionDict.TryGetValue(ans.OptionId.Value, out var opt))
                {
                ...
                else if (!string.IsNullOrEmpty(ans.ExtraOptionIds))
                {
                    try
                    {
                        var ids = System.Text.Json.JsonSerializer.Deserialize<List<int>>(ans.ExtraOptionIds);
                        if (ids != null)
                        {
                            var contents = ids.Where(optionDict.ContainsKey).Select(id => optionDict[id].Content);
                            answerContent = string.Join(", ", contents);
                        }
                    }
                    catch { }
                }
        ...
        public async Task<List<SurveyResponseDto>> GetSurveyResponsesAsync(int surveyId)
        {
            ...
            foreach (var resp in responses)
            {
                ...
                {
                    var q = ans.QuestionId.HasValue ? questionDict.GetValueOrDefault(ans.QuestionId.Value) : null;
                    var questionType = q?.QuestionType;
                    var answerDto = new SurveyAnswerResultDto
                    {
                        QuestionId = ans.QuestionId ?? 0,
                        QuestionContent = q?.Content ?? ans.QuestionContent ?? "",
