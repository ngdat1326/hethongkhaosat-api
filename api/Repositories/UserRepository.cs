...
namespace api.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<int> BulkDeleteUsersAsync(BulkUserActionRequest request)
        {
            var users = await GetUsersByRequest(request);
            int count = 0;
            foreach (var user in users)
            {
                var result = await _userManager.DeleteAsync(user);
            ...
            public async Task<int> BulkSetActiveStatusAsync(BulkSetActiveRequest request)
            {
                var users = await GetUsersByRequest(request);
                int count = 0;
                foreach (var user in users)
            ...
            private async Task<List<AppUser>> GetUsersByRequest(BulkUserActionRequest request)
            {
                ...
                {
                    return new List<AppUser>(); // No users if no IDs provided
                }
