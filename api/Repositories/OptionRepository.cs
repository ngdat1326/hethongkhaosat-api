...
namespace api.Repositories
{
    public class OptionRepository : IOptionRepository
    {
        ...
        // Thay th? toàn b? options cho m?t câu h?i
        public async Task<bool> ReplaceOptionsForQuestionAsync(int questionId, List<CreateOptionDto> newOptions)
        {
            // Xóa t?t c? options c?
            var oldOptions = _context.Options.Where(o => o.QuestionId == questionId);
            _context.Options.RemoveRange(oldOptions);
            await _context.SaveChangesAsync();
            // Thêm các options m?i
            foreach (var dto in newOptions)
            {
                var o = new Option
                {
