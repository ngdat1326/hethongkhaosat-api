...
namespace api.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        private const int OtpResendLimit = 3;
        private const int OtpResendWindowMinutes = 60;

        public AuthController(UserManager<AppUser> userManager, RoleManager<IdentityRole> roleManager, TokenService tokenService, ApplicationDbContext context, EmailService emailService)
        {
        ...
        public async Task<IActionResult> Login(LoginDto dto)
        {
            ...
            if (!user.IsEmailVerified)
            {
                ...
                var expiry = DateTime.UtcNow.AddMinutes(3); // Đổi thành 3 phút
        ...
        {
            var subject = "Mã xác thực OTP đăng nhập";
            var body = $"<p>Mã OTP của bạn là: <b>{otp}</b></p><p>Mã có hiệu lực trong 3 phút.</p>";
        ...
        public async Task<IActionResult> ResendOtp([FromBody] ResendOtpDto dto)
        {
            ...
            if (user == null) return Unauthorized();
            if (user.IsEmailVerified)
                return BadRequest(new { message = "Tài khoản đã xác thực email, không cần gửi lại OTP." });

            // Đếm số lần gửi lại OTP trong 1 giờ qua
            var since = DateTime.UtcNow.AddMinutes(-OtpResendWindowMinutes);
            var resendCount = await _context.UserOtps.CountAsync(x => x.UserId == user.Id && x.Expiry > since);
            if (resendCount >= OtpResendLimit)
                return BadRequest(new { message = $"Bạn đã gửi lại OTP quá số lần cho phép. Vui lòng thử lại sau." });

            // Đánh dấu tất cả OTP cũ chưa dùng là đã dùng
            var oldOtps = await _context.UserOtps.Where(x => x.UserId == user.Id && !x.IsUsed).ToListAsync();
            foreach (var oldOtp in oldOtps)
            {
                oldOtp.IsUsed = true;
            }
            _context.UserOtps.UpdateRange(oldOtps);
