...
namespace api.Repositories
{
    public class QuestionRepository : IQuestionRepository
    {
        ...
        // ?? quy l?y t?t c? các questionId con c?a m?t câu h?i g?c
        public async Task<List<int>> GetAllDescendantQuestionIdsAsync(int rootId)
        {
            var result = new List<int>();
            var directChildren = await _context.QuestionBranches
                .Where(b => b.CurrentQuestionId == rootId)
                .Select(b => b.NextQuestionId)
                .ToListAsync();
            foreach (var childId in directChildren)
            {
                result.Add(childId);
                var subChildren = await GetAllDescendantQuestionIdsAsync(childId);
        ...
        {
            // ?? quy xóa t?t c? các questionId con c?a m?t câu h?i g?c
            var allDescendants = await GetAllDescendantQuestionIdsAsync(id);
            foreach (var childId in allDescendants.Distinct())
            {
                await DeleteAsync(childId);
            }
            // Set null OptionId trong Answer tr??c khi xóa Option c?a Question này
            var optionIds = await _context.Options.Where(o => o.QuestionId == id).Select(o => o.Id).ToListAsync();
            if (optionIds.Any())
            {
                var answers = _context.Answers.Where(a => a.OptionId != null && optionIds.Contains(a.OptionId.Value));
                await answers.ForEachAsync(a => a.OptionId = null);
                await _context.SaveChangesAsync();
            }
            // Set null QuestionId trong Answer tr??c khi xóa Question
            var answersWithQuestion = _context.Answers.Where(a => a.QuestionId == id);
            await answersWithQuestion.ForEachAsync(a => a.QuestionId = null);
            await _context.SaveChangesAsync();
            // Xóa các branch liên quan
        ...
        public async Task UpdateOrderAsync(List<UpdateQuestionOrderDto> dtos)
        {
            var ids = dtos.Select(d => d.Id).ToList();
            var questions = await _context.Questions.Where(q => ids.Contains(q.Id)).ToListAsync();
            foreach (var dto in dtos)
            {
                var q = questions.FirstOrDefault(x => x.Id == dto.Id);
                if (q != null)
                {
                    q.Order = dto.Order;
                }
