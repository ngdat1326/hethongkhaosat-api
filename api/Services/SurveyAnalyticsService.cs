    ...
    public class SurveyAnalyticsService
    {
        ...
        public List<SurveyAnalyticsQuestionDto> GetSurveyAnalytics(int surveyId)
        {
            ...
            foreach (var q in questions)
            {
                ...
                {
                    // ??m s? answer có ít nh?t 1 l?a ch?n (OptionId, OptionIdBackup, ExtraOptionIds, ExtraOptionIdsBackup)
                ...
                else if (q.QuestionType.Code == "MultiChoice")
                {
                    ...
                    foreach (var opt in options)
                    {
                        // ??m s? answer duy nh?t mà option này xu?t hi?n (? b?t k? tr??ng nào)
                        int count = qAnswers.Count(a =>
                            a.OptionId == opt.Id ||
                            a.OptionIdBackup == opt.Id ||
                            ParseMultiOptionIds(a.ExtraOptionIds).Contains(opt.Id) ||
                            ParseMultiOptionIds(a.ExtraOptionIdsBackup).Contains(opt.Id)
                        );
                        double percent = totalAnswers > 0 ? (count * 100.0 / totalAnswers) : 0;
                        optionStats.Add(new SurveyAnalyticsOptionDto
                        {
        ...
        // Helper: parse chu?i ki?u [255,256] thành List<int>
