    ...
    public class PublicSurveyController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetSurveyDetail(int surveyId)
        {
            try
            {
                ...
                QuestionTreeNodeDto? BuildTree(int questionId, int? parentQuestionId = null)
                {
                    ...
                    var node = new QuestionTreeNodeDto
                    {
                        ...
                        ParentQuestionId = parentQuestionId // Thêm parentQuestionId
                    };
                    if (optionDict.TryGetValue(q.Id, out var options))
                    {
                        // Gộp các branch có cùng nextQuestionId, conditionType, branchOptionIds
                        var branchGroups = new List<(int? nextQid, string? conditionType, List<int>? branchOptionIds, List<int> optionIds)>();
                        if (branchDict.TryGetValue(q.Id, out var branches))
                        {
                            branchGroups = branches
                                .GroupBy(b => new { NextQuestionId = (int?)b.NextQuestionId, ConditionType = b.ConditionType, Key = string.Join(",", b.OptionIds.OrderBy(x => x)) })
                                .Select(g => (
                                    nextQid: g.Key.NextQuestionId,
                                    conditionType: g.Key.ConditionType,
                                    branchOptionIds: g.First().OptionIds,
                                    optionIds: g.SelectMany(b => b.OptionIds).Distinct().ToList()
                                )).ToList();
                        }
                        foreach (var opt in options)
                        {
                            ...
                            List<int>? branchOptionIds = null;
                            // Tìm group branch mà option này thu?c v?
                            var group = branchGroups.FirstOrDefault(bg => bg.optionIds.Contains(opt.Id));
                            if (group.nextQid.HasValue)
                            {
                                // Ch? gán nextQuestion cho option ??i di?n ??u tiên trong group
                                if (group.branchOptionIds != null && group.branchOptionIds.Count > 0 && group.branchOptionIds[0] == opt.Id)
                                {
                                    nextQid = group.nextQid;
                                }
                                conditionType = group.conditionType;
                                branchOptionIds = group.branchOptionIds;
                            }
                            node.Options.Add(new OptionTreeNodeDto
                            {
                                ...
                                Content = opt.Content,
                                Value = opt.Value,
                                NextQuestion = nextQid.HasValue ? BuildTree(nextQid.Value, q.Id) : null,
                                ConditionType = conditionType,
                                BranchOptionIds = branchOptionIds
                            });
                        }
                    }
                    return node;
                ...
                {
                    var rootNode = BuildTree(rootId, null);
                    if (rootNode != null)
        ...
        public async Task<IActionResult> SubmitSurvey([FromBody] SubmitSurveyResultDto dto)
        {
            ...
            await _responseService.SaveSurveyResultAsync(dto);
        ...
        {
            var responses = await _responseService.GetSurveyResponsesAsync(surveyId);
            return Ok(responses);
        }
        ...
        public async Task<IActionResult> GetActiveSurveys()
        {
            ...
            var activeSurveys = allSurveys
                .Where(s => s.Status == 1 && s.StartDate <= now && s.EndDate >= now)
                .ToList();
